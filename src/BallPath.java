...
public class BallPath extends Entity {
    ...
    public BallPath(int x, int y, int vx, int vy, int radius, Path path) {
    ...
    public void move() {
    	if(dir == 1 && path.getPath().size() > 1) {
   			Rectangle next = new Rectangle((int)path.getPoint(curr+1).getX()-5,(int)path.getPoint(curr+1).getY()-5,10,10);
   			if(!hitbox.intersects(next)) {
   				if(next.getCenterX() > x) {
   	   				x -= vx;
   	   			}
   	   			if(next.getCenterX() < x) {
   	   				x += vx;
   	   			}
   	   			if(next.getCenterY() > y) {
   	   				y -= vy;
   	   			}
   	   			if(next.getCenterY() < y) {
   	   				y += vy;
